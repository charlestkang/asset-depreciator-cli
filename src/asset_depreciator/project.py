...
def get_float_optional(prompt: str = "", default=0.0) -> float:
    while True:
        s = input(prompt)
        try:
            if not s.strip():
...
def calculate_view_totals(assets: list[Asset]) -> tuple[float, float, float]:
    """Return aggregate live depreciation rates across all assets."""
    daily_total = 0.0
    yearly_total = 0.0
    for asset in assets:
        metrics = calculator.build_metrics(asset)
        daily_total += metrics.daily_depr
        yearly_total += metrics.yearly_depr
    monthly_total = yearly_total / 12
    return daily_total, monthly_total, yearly_total
...
def view_screen() -> None:
    while True:
        ...
        daily_total, monthly_total, yearly_total = calculate_view_totals(assets)
        display.display_view_totals(daily_total, monthly_total, yearly_total)
        try:
    ...
    while True:
        ...
        match intention:
            case "add":
                ...
                except storage.DuplicateAssetError as e:
                    print(f"\n{e}")
                print()
            case "view":
                view_screen()
